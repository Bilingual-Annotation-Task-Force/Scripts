...
# Define methods
def get_py_dict():  # for default D2 argument in OOV_remove
...
def condition3(word, d2):
    if type(d2) in {set, list, tuple, dict}:
        ...
        # This line would print to the console on each OOV if the STDOUT were not changed.
        return d2.meaning(word) is None


def oov_remove(tokens, d1, d2=get_py_dict()):
...
def get_txt(file_name, encoding_type=None):
    """Reads and splits a .txt files. Appends ".txt" to file name if necessary."""
    name = file_name
    if name[-4:] != ".txt":
        name += ".txt"
...
def destwrite(words, help_message):
    ...
    while option in {'c', 'n'}:  # determine how to open file
        ...
        # User should preferably type a file name that does not already exist, in which case this block is not
        # necessary.
        try:

            dest = open(destname, 'r')
            print("\nFile by that name already exists.")
            prompt = 'Options:\n\t"O" - overwrite contents\n\t"A" - append to contents\n\t"C" - create new file with ' \
                     '"(1)" appended to name\n\t"N" - enter new name\n\t[ctrl]+[C] - exit\n\nInput: '
...
if __name__ == "__main__":
    ...
    parser.add_argument("D2",
                        help='Name of the language 2 dictionary .TXT file. Enter "." for PyDictionary (requires '
                             'PyDictionary module and reliable internet connection). NOTE: PyDictionary only for '
                             'English; English dictionary must be D2 if using PyDictionary.')
    parser.add_argument("-t", "--TOKENS_encoding",
                        help="Tokens .TXT file encoding type. Default used if not specified.")
    parser.add_argument("-d1", "--D1_encoding",
                        help="Language 1 dictionary .TXT file encoding type. Default used if not specified.")
    parser.add_argument("-d2", "--D2_encoding",
                        help="Language 2 dictionary .TXT file encoding type. Default used if not specified.")
    parser.add_argument("-cd", "--change_directory",
                        help='Change the folder in which to locate .TXT files. NOTE: It is also possible to specify '
                             'individual file locations by including the entire path starting from "C:\".')
    ...
    tokens = get_txt(args.TOKENS, args.TOKENS_encoding)
    D1 = get_txt(args.D1, args.D1_encoding)
    ...
    if args.D2 == ".":
        ...
        D2 = get_txt(args.D2, args.D2_encoding)

    print("\nRemoving OOVs...\n")
    (tokens_without_OOVs, OOVs) = oov_remove(tokens, D1, D2)
    print("\nOOVs removed.\n")

    help_message = '\nDestination .TXT file used to store tokens list after removing out-of-vocabulary (OOV) words, ' \
                   'a.k.a. "mixed words". If destination file to be outside of current working directory, ' \
                   'include file location path in name. '
    ...
    if keep_OOVs.lower() == 'y':
        help_message = '\nDestination .TXT file used to store removed out-of-vocabulary (OOV) words, a.k.a. "mixed ' \
                       'words", and their corresponding locations in the original tokens list. If destination file to ' \
                       'be outside of current working directory, include file location path in name. '
